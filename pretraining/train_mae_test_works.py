...
def main():
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.000164746*16, weight_decay=0.0, betas=(0.845, 0.985)) # lr = 1e-5
    ...
    start_epoch, effective_step_count, best_loss, total_loss = 0, 0, float('inf'), 0 #load_checkpoint(model, optimizer, scheduler)
   # model = nn.DataParallel(model)
            ...
            if ((i + 1) % accumulate_steps == 0) or (i+1 == len(train_dataloader)):     
                ...
                if save_checkpoint_flag:
                    ...
                    if avg_val_loss < best_loss:
                        ...
                        torch.save(model.module.state_dict(), "best_model_test.pth")
